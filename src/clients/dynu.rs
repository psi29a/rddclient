...
/// Dynu DNS client
/// Uses DynDNS2 protocol with basic auth or API key
...
impl DynuClient {
    pub fn new(config: &Config) -> Result<Self, Box<dyn Error>> {
        ...
        let password = config.password.as_ref()
            .ok_or("password is required for Dynu")?
            .clone();
        
        let server = config.server.as_ref()
            .cloned()
            .unwrap_or_else(|| "https://api.dynu.com".to_string());
...
impl DnsClient for DynuClient {
    fn update_record(&self, hostname: &str, ip: IpAddr) -> Result<(), Box<dyn Error>> {
        ...
        log::info!("Updating {} with Dynu", hostname);
    ...
    fn validate_config(&self) -> Result<(), Box<dyn Error>> {
        ...
        if self.password.is_empty() {
            return Err("password is required for Dynu".into());
        }
        Ok(())
    }
